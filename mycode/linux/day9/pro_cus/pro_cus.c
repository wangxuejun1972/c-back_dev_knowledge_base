...
int main(int argc,char* argv[])
{
    ...
    unsigned short arr[2]={10,0};//0号信号量的值代表仓库可用位置数目，1号信号量的值代表产品个数
    semctl(semArrId,0,SETVAL,arr);
    ...
    if(!fork()) {
        while(1) {
            ...
            printf("Consume process,repository num=%d,product num=%d\n",semctl(semArrId,0,GETVAL),semctl(semArrId,1,GETVAL));
        ...
        while(1) {
            ...
            sopv.sem_op=1;//进行V操作
            sopv.sem_flg=SEM_UNDO;
            printf("Producer process,repository num=%d,product num=%d\n",semctl(semArrId,0,GETVAL),semctl(semArrId,1,GETVAL));
